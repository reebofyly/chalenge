...
# Objectif : Télécharger, nettoyer et structurer les données de scolarisation primaire brut du Bénin
# Source : Banque Mondiale - Indicateur SE.PRM.ENRR (https://api.worldbank.org/v2/en/indicator/SE.PRM.ENRR?downloadformat=csv)
# Auteur : SOULE Fadile
# Date : 20/09/25
# =========================================================
...
# === ÉTAPE 1 : Téléchargement du fichier ZIP depuis l'API de la Banque Mondiale ===
url = "https://api.worldbank.org/v2/en/indicator/SE.PRM.ENRR?downloadformat=csv"

print("Téléchargement du fichier ZIP depuis la Banque Mondiale...")
response = requests.get(url)
response.raise_for_status()  # Lève une exception en cas d'erreur HTTP

# Sauvegarde locale du fichier ZIP
zip_path = os.path.join(DATA_RAW_DIR, "wb_school_enrollment.zip")
...
with zipfile.ZipFile(zip_path, "r") as z:
    ...
    data_file = None
    for filename in file_list:
        if filename.startswith("API_SE.PRM.ENRR") and "Metadata" not in filename and filename.endswith(".csv"):
            data_file = filename
            break

    if not data_file:
        raise FileNotFoundError(
            "Aucun fichier de données principal (CSV) trouvé dans le ZIP. "
            "Vérifiez la structure du fichier téléchargé."
        )
...
print(f"Dimensions du jeu de données : {df.shape[0]} pays x {df.shape[1]} colonnes")
...
df_benin_long = df_benin_clean.melt(
    ...
    var_name="Année",
    value_name="Taux_Scolarisation_Primaire"
)
...
df_benin_long = df_benin_long.dropna(subset=["Taux_Scolarisation_Primaire"])
...
output_path = os.path.join(DATA_CLEANED_DIR, "wb_school_enrollment_benin.csv")
