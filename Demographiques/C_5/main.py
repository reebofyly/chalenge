...
# Objectif : Télécharger, nettoyer et structurer les données sur les enseignants hommes formés dans l'enseignement primaire au Bénin
# Source : Banque Mondiale - Indicateur SE.PRM.TCAQ.MA.ZS (https://api.worldbank.org/v2/en/indicator/SE.PRM.TCAQ.MA.ZS?downloadformat=csv)
# Auteur : SOULE Fadile
# Date : 20/09/25
# =========================================================
...
indicator_code = "SE.PRM.TCAQ.MA.ZS"  # % d'enseignants hommes formés dans l'enseignement primaire
url = f"https://api.worldbank.org/v2/en/indicator/SE.PRM.TCAQ.MA.ZS?downloadformat=csv"
...
zip_path = os.path.join(DATA_RAW_DIR, f"wb_teachers_male_trained_benin.zip")
...
df_benin_long = df_benin_clean.melt(
    ...
    var_name="Année",
    value_name="Pourcentage_Enseignants_Hommes_Formes_Primaire"
)
...
# Suppression des valeurs manquantes pour le pourcentage d’enseignants hommes formés
df_benin_long = df_benin_long.dropna(subset=["Pourcentage_Enseignants_Hommes_Formes_Primaire"])
...
df_benin_final = df_benin_long[[
    ...
    "Country Code",
    "Année",
    "Pourcentage_Enseignants_Hommes_Formes_Primaire"
]]
...
output_path = os.path.join(DATA_CLEANED_DIR, "wb_teachers_male_trained_primary_benin.csv")
