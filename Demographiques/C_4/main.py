...
# Objectif : Télécharger, nettoyer et structurer les données sur les enseignantes formées dans l'enseignement primaire au Bénin
# Source : Banque Mondiale - Indicateur SE.PRM.TCAQ.FE.ZS (https://api.worldbank.org/v2/en/indicator/SE.PRM.TCAQ.FE.ZS?downloadformat=csv)
# Auteur : SOULE Fadile
# Date : 20/09/25
# =========================================================
...
indicator_code = "SE.PRM.TCAQ.FE.ZS"  # % d'enseignantes formées dans l'enseignement primaire
url = f"https://api.worldbank.org/v2/en/indicator/SE.PRM.TCAQ.FE.ZS?downloadformat=csv"
...
zip_path = os.path.join(DATA_RAW_DIR, f"wb_teachers_female_trained_benin.zip")
...
df_benin_long = df_benin_clean.melt(
    ...
    var_name="Année",
    value_name="Pourcentage_Enseignantes_Formees_Primaire"
)
...
# Suppression des valeurs manquantes pour le pourcentage d’enseignantes formées
df_benin_long = df_benin_long.dropna(subset=["Pourcentage_Enseignantes_Formees_Primaire"])
...
df_benin_final = df_benin_long[[
    ...
    "Country Code",
    "Année",
    "Pourcentage_Enseignantes_Formees_Primaire"
]]
...
output_path = os.path.join(DATA_CLEANED_DIR, "wb_teachers_female_trained_primary_benin.csv")
