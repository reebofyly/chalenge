...
# Objectif : Télécharger, nettoyer et structurer les données sur les enseignants formés dans l'enseignement primaire au Bénin
...
indicator_code = "SE.PRM.TCAQ.MA.ZS"  # % d'enseignants formés dans l'enseignement primaire
...
zip_path = os.path.join(DATA_RAW_DIR, f"wb_teachers_trained_benin.zip")
...
df_benin_long = df_benin_clean.melt(
    ...
    var_name="Année",
    value_name="Pourcentage_Enseignants_Formes_Primaire"
)
...
# Suppression des valeurs manquantes pour le pourcentage d’enseignants formés
df_benin_long = df_benin_long.dropna(subset=["Pourcentage_Enseignants_Formes_Primaire"])
...
df_benin_final = df_benin_long[[
    ...
    "Country Code",
    "Année",
    "Pourcentage_Enseignants_Formes_Primaire"
]]
...
output_path = os.path.join(DATA_CLEANED_DIR, "wb_teachers_trained_primary_benin.csv")
